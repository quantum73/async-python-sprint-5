...
async def files_list(
    ...
    files = await files_services.get_user_files(db, user_id=user_id.hex, skip=paginator.offset, limit=paginator.limit)
    return {
        "account_id": user_id,
        "files": files,
...
async def upload_file(
    ...
    file_in_db = await files_services.create_file(
        db,
...
async def download_file(
    ...
    file_obj = await files_services.get_file_by_id_or_path(db, value=path)
    if file_obj is None:
...
async def search_files(
    ...
    files = await files_services.search_files(db, search_options=search_options)
